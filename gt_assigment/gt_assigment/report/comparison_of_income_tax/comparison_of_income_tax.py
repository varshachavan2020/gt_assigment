...
class IncomeTaxComputationReport:
	...
	def get_applicable_tax(self):
		...
		self.add_column("(New Regime) Income Tax (Slab Based)", "new_income_tax_slab_based")
		self.add_column("(New Regime) Other Taxes and Charges", "new_other_taxes_and_charges")
		self.add_column("(New Regime) Total Applicable Tax", "new_applicable_tax")
		...
		for emp, emp_details in self.employees.items():
			tax_slab = frappe.get_cached_doc("Income Tax Slab", "Old Regime FY2025")
			...
			tax_slab = frappe.get_cached_doc("Income Tax Slab", "New Regime FY2025")
			eval_globals, eval_locals = self.get_data_for_eval(emp, emp_details)
			tax_amount, other_taxes_and_charges = calculate_tax_by_tax_slab(
				emp_details["total_taxable_amount"],
				tax_slab,
			...
			emp_details["new_income_tax_slab_based"] = tax_amount - other_taxes_and_charges
			emp_details["new_other_taxes_and_charges"] = other_taxes_and_charges
			emp_details["new_applicable_tax"] = tax_amount
	...
	def get_payable_tax(self):
		self.add_column("Old Payable Tax", "payable_tax" )
		self.add_column("(New Regime) Payable Tax", "new_payable_tax")
		...
		for __, emp_details in self.employees.items():
			...
			new_payable_tax = flt(emp_details.get("new_applicable_tax")) - flt(emp_details.get("total_tax_deducted"))
			if new_payable_tax < 0:
				new_payable_tax = 0.0
			emp_details["new_payable_tax"] = new_payable_tax
